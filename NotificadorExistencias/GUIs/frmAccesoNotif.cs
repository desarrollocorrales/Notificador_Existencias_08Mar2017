...
{
    public partial class frmAccesoNotif : Form
    {
        public frmAccesoNotif()
        {
            InitializeComponent();
        }

        private void frmAccesoNotif_Load(object sender, EventArgs e)
        {
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string acceso = Properties.Settings.Default.accesoNotif;
        ...
        private void btnCambCve_Click(object sender, EventArgs e)
        {
            ...
            this.tbPCveAdmin.Text = string.Empty;
        ...
        private void btnPAceptar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(this.tbPCveAdmin.Text))
                    throw new Exception("Ingrese la clave del Administrador");
                ...
                // valida si la clave del administrador es correcta
                string claveAdmin = this.tbPCveAdmin.Text;
                string accesoAdmin = Properties.Settings.Default.accesoConfig;

                if (!Modelos.Utilerias.Transform(accesoAdmin).Equals(Modelos.Utilerias.Transform(claveAdmin)))
                {
                    this.ActiveControl = this.tbPCveAnt;
                    this.tbPCveAnt.SelectAll();
                    throw new Exception("Clave del Administrador es incorrecta");
                }

                string cveNAcceso = Modelos.Utilerias.Transform(this.tbPCveNueva.Text);

                Properties.Settings.Default.accesoNotif = cveNAcceso;
